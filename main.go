...
import (
	...
	"github.com/Pyxis66/YallyScreen/ui"
...
func init() {
	logger.Debug("-")
	logger.Debug("-")
	logger.TraceEnter("YallyScreen - main.init()")
	initSucceeded = false
	...
	initSucceeded = true
	logger.TraceLeave("YallyScreen - main.init()")
	logger.Debug("-")
	logger.Debug("-")
}
...
func main() {
	...
	logger.Debug("+")
	logger.Debug("+")
	logger.TraceEnter("YallyScreen - main.main()")
	...
	if initSucceeded != true {
		...
		if utils.RequiredEnvironmentVariablesAreSet(APIKey) {
			...
			if err == nil {
				// width and height come from EnvResolution/YALLYSCREEN_RESOLUTION
				...
				// But if there is an error while parsing YALLYSCREEN_RESOLUTION,
	...
	logger.TraceLeave("YallyScreen - main.main()")
	logger.Debug("+")
	logger.Debug("+")
}
...
func setCursor() {
	...
	// 	https://github.com/Pyxis66/YallyScreen/issues/285
