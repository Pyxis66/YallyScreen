...
func (this *UI) loadSettings() {
	...
	settingsResponse, err := (&octoprintApis.YallyScreenSettingsRequest{}).Do(this.Client, this.UIState)
	if err != nil {
		text := err.Error()
		if strings.Contains(strings.ToLower(text), "unexpected status code: 404") {
			...
			// YallyScreen plug-in is available.  If calling GetSettings returns
			// a 404, the plug-in isn't available.
			logger.Info("The YallyScreen plug-in is not available")
		} else {
		...
		this.Settings = &dataModels.YallyScreenSettingsResponse {
